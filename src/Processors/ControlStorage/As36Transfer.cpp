...
bool As36ControlStorageProcessor::copyCallerAddressability(int callerRb, int calleeRb, int calleePb, const std::string& call)
{
    ...
    if ((mode & 0xA0) == 0xA0) {
        // nuprblen (c18a64d8..c18a64e4) reserves no caller-map room for an
        // A0-class callee and nup2000 does not call nucmclr for it: such a
        // program sees SLIC low storage through ATR 1 and its own module,
        // plus whatever its header MAP list and SVC 2F build.  Copying the
        // caller's map here hid real page 1 behind the caller's region and
        // fed #DPDM, SPALC and #CAS1 zeros for $0B97/$0B9B.
        return false;
    }
