                            ...
                            const HostInfo& host, uint8_t systemCustomize1, std::string& error)
{
    ...
    // of stations in the machine definition.  The UDT controller record below
    // refreshes it; seeding it here also keeps the value available if a
    // malformed, nonblank third-party UDT omits that record.
    m.writeByte(kMaxDevices08C3,
                static_cast<uint8_t>(devices::WorkStationController::kMaxDevices));
    trace.csp("low storage: {:04X} = {} - emulator work-station controller maximum capacity, "
...
std::vector<uint8_t> GuestLowStorage::synthesizeUnitDefinitionTable(uint8_t systemCustomize1)
{
    // These records are the power-on descriptions of hardware SIM/36 really
    // implements: system, work-station controller, fixed disk, diskette,
    // tape, system console and the Advanced/36 ELCA controller.  Record
    // layouts and fixed fields are taken byte-for-byte from an Advanced/36
    // UDT; only the model-derived system customize byte is variable.
    std::vector<uint8_t> udt(4096, 0);
    std::vector<uint8_t> records = {
        
    };
    records[11] = systemCustomize1;
    std::copy(records.begin(), records.end(), udt.begin());
    return udt;
}
