// EBCDIC code page tables.  Generated by tools/gen-ebcdic.py from the
// standard cp500 (EBCDIC International) and cp037 (EBCDIC US/Canada)
// definitions; every byte maps to one Latin-1 code point.
...
struct Reverse {
    uint8_t byte[256];   // Latin-1 code point -> EBCDIC byte
    bool valid[256];
};

Reverse buildReverse(const uint16_t* forward)
{
    Reverse r{};
    std::memset(r.valid, 0, sizeof r.valid);
    for (int b = 0; b < 256; ++b) {
        uint16_t cp = forward[b];
        if (cp < 256 && !r.valid[cp]) {
            r.byte[cp] = static_cast<uint8_t>(b);
            r.valid[cp] = true;
        }
    }
    return r;
...
const Reverse& reverseFor(const uint16_t* forward)
{
    static const Reverse cp500 = buildReverse(kCp500);
    static const Reverse cp037 = buildReverse(kCp037);
    return forward == kCp500 ? cp500 : cp037;
}

const uint16_t* forwardFor(Ebcdic::CodePage page)
{
    return page == Ebcdic::CodePage::Cp037 ? kCp037 : kCp500;
}
...
uint32_t nextCodePoint(const std::string& s, std::size_t& i)
{
    ...
    for (std::size_t k = 1; k <= extra; ++k) {
        ...
        if ((b & 0xC0) != 0x80) { ++i; return 0xFFFD; }
        cp = (cp << 6) | (b & 0x3F);
    }
    i += extra + 1;
    return cp;
...
uint16_t Ebcdic::toUnicode(uint8_t b, CodePage page) { return forwardFor(page)[b]; }

std::string Ebcdic::toAscii(const uint8_t* bytes, std::size_t length, CodePage page)
{
    const uint16_t* forward = forwardFor(page);
    std::string s;
    s.reserve(length);
    for (std::size_t i = 0; i < length; ++i) appendUtf8(s, forward[bytes[i]]);
    return s;
}

std::string Ebcdic::toAscii(const std::vector<uint8_t>& bytes, std::size_t offset,
                            std::size_t length, CodePage page)
{
    if (offset > bytes.size()) return std::string();
    if (offset + length > bytes.size()) length = bytes.size() - offset;
    return toAscii(bytes.data() + offset, length, page);
}

uint8_t Ebcdic::fromChar(uint32_t codePoint, CodePage page)
{
    const Reverse& r = reverseFor(forwardFor(page));
    if (codePoint < 256 && r.valid[codePoint]) return r.byte[codePoint];
    return kSubstitute;
}

std::vector<uint8_t> Ebcdic::fromAscii(const std::string& utf8, CodePage page)
{
    std::vector<uint8_t> out;
    out.reserve(utf8.size());
    std::size_t i = 0;
    while (i < utf8.size()) out.push_back(fromChar(nextCodePoint(utf8, i), page));
    return out;
}
